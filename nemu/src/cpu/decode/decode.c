...
/* 0f 20 /r: mov r32, crN (dest=rm(gpr), src=reg(cr)) */
make_DHelper(cr2r) {
  decode_op_rm(eip, id_dest, false, id_src, false);
  assert(id_dest->type == OP_TYPE_REG && id_src->type == OP_TYPE_REG);
}

/* 0f 22 /r: mov crN, r32 (dest=reg(cr), src=rm(gpr)) */
make_DHelper(r2cr) {
...
/* Ev <- Gv, CL : the CL-count form of shld/shrd */
make_DHelper(cl_G2E) {
  decode_op_rm(eip, id_dest, true, id_src2, true);
  id_src->type = OP_TYPE_REG;
  id_src->reg = R_CL;
