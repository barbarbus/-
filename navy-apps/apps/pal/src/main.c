...
main_loop() {
  ...
  wScreenHeight = 200;
   ...
   /* Skip the long opening animation (~32s of RNG + fade) so we can verify
    * the renderer / event loop on NEMU; re-enable later for the final demo. */
#ifdef PAL_PLAY_INTRO
   PAL_TrademarkScreen();
   PAL_SplashScreen();
#endif
