...
#else
   /* Indexed pixels live on gpScreen; keep palette pointer consistent for PAL_FadeOut etc. */
   assert(gpScreen != NULL && gpScreen->format != NULL &&
       gpScreen->format->palette != NULL);
   return gpScreen->format->palette->colors;
#endif
}
